<template>
    <div class="border-b border-gray-200 pb-4 mb-4">
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-x-3 gap-y-3 items-end">
            ...
            <div>
                ...
                <select id="status-selector" v-model="localFilters.status"
                    class="mt-0 block w-full pl-2 pr-8 py-1.5 h-8 text-xs border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 rounded-md shadow-sm">
            ...
            <div>
                ...
                <input type="date" id="date-from" v-model="localFilters.desde"
                    class="mt-0 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-xs h-8 px-2 py-1.5" />
            </div>
            ...
            <div>
                <input type="text" v-model.lazy="localFilters.search_idamostra" placeholder="Id. Amostra" class="simple-input w-full" />
            </div>

            <div>
                <input type="text" v-model.lazy="localFilters.search_cdamostra" placeholder="Código Amostra" class="simple-input w-full" />
            </div>

            <div>
                <input type="text" v-model.lazy="localFilters.search_solicitante" placeholder="Solicitante" class="simple-input w-full" />
            </div>


            <div>
                <input type="text" v-model.lazy="localFilters.search_tipo" placeholder="Matriz (Tipo)" class="simple-input w-full" />
            </div>

            <div>
                <input type="text" v-model.lazy="localFilters.search_grupo" placeholder="Grupo" class="simple-input w-full" />
            </div>

            <div>
                <input type="text" v-model.lazy="localFilters.search_processo" placeholder="Proceso" class="simple-input w-full" />
            </div>

            <div>
                <input type="text" v-model.lazy="localFilters.search_numero" placeholder="Número" class="simple-input w-full" />
            </div>

            <div class="flex items-end space-x-2 col-span-1 sm:col-span-2 md:col-span-2 lg:col-span-2">
...
<script setup>
// Tu script setup permanece igual que en la respuesta anterior con el método resetFilters
...
const initializeFilters = () => {
    localFilters.status = props.initialFilters.status || "4";
...
const resetFilters = () => {
    ...
    console.log("Filtros reseteados. Emitiendo:", { ...localFilters });
    triggerEmitFilters();
};
</script>
...
<style scoped>
.simple-input {
    /* Cambio: py-1.5 para consistencia vertical con otros inputs */
    @apply block w-full border border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-xs h-8 px-2 py-1.5;
}
