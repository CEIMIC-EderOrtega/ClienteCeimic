...
// --- NUEVA PROPIEDAD COMPUTADA PARA COMPROBAR ROLES ---
const page = usePage();
const userRoles = computed(() => page.props.auth.user.roles || []); // Asegura que sea un array vacío si no hay roles
const isAdmin = computed(() => userRoles.value.includes('Administrador'));
// --- FIN NUEVA PROPIEDAD COMPUTADA ---
</script>
                            ...
                            <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                                ...
                                <template v-if="isAdmin">
                                    ...
                                    <NavLink :href="route('admin.roles.index')"
                                        :active="route().current('admin.roles.*')" :class="{
                                            'text-white': true,
                                            'hover:bg-white/10 focus:bg-white/10': true,
                                    ...
                                    <NavLink :href="route('admin.users.index')"
                                        :active="route().current('admin.users.*')" :class="{
                                            'text-white': true,
                                            'hover:bg-white/10 focus:bg-white/10': true,
                ...
                <div :class="{ block: showingNavigationDropdown, hidden: !showingNavigationDropdown }"
                    ...
                    <div class="space-y-1 pb-3 pt-2">
                        <ResponsiveNavLink :href="route('dashboard')" :active="route().current('dashboard')" :class="{
                            ...
                            'bg-white/20': route().current('dashboard'),
                            'text-white': !route().current('dashboard'),
                            'text-gray-800': route().current('dashboard'),
                            'hover:bg-white/10 focus:bg-white/10': true,
                            'hover:text-gray-900 focus:text-gray-900': true,
                        }">
                            <MagnifyingGlassIcon class="h-5 w-5 mr-1"
                                :class="{ 'text-gray-800': route().current('dashboard'), 'text-white': !route().current('dashboard') }" />
                            Muestras
                        </ResponsiveNavLink>
                        ...
                        <template v-if="isAdmin">
                            <ResponsiveNavLink :href="route('admin.countries.index')"
                                ...
                                <GlobeAmericasIcon class="h-5 w-5 mr-1"
                                    :class="{ 'text-gray-800': route().current('admin.countries.index'), 'text-white': !route().current('admin.countries.index') }" />
                                Paises
                            </ResponsiveNavLink>

                            <ResponsiveNavLink :href="route('admin.roles.index')"
                                :active="route().current('admin.roles.*')" :class="{
                                    'flex items-center px-4 py-2': true,
                                ...
                                <FingerPrintIcon class="h-5 w-5 mr-1"
                                    :class="{ 'text-gray-800': route().current('admin.roles.*'), 'text-white': !route().current('admin.roles.*') }" />
                                Roles
                            </ResponsiveNavLink>
                            ...
                            <ResponsiveNavLink :href="route('admin.companies.index')"
                                ...
                                <BuildingOffice2Icon class="h-5 w-5 mr-1"
                                    :class="{ 'text-gray-800': route().current('admin.companies.*'), 'text-white': !route().current('admin.companies.*') }" />
                                Empresas
                            </ResponsiveNavLink>
                            ...
                            <ResponsiveNavLink :href="route('admin.users.index')"
                                ...
                                <UserIcon class="h-5 w-5 mr-1"
                                    :class="{ 'text-gray-800': route().current('admin.users.*'), 'text-white': !route().current('admin.users.*') }" />
                                Usuarios
                            </ResponsiveNavLink>
                        </template>
                    </div>
